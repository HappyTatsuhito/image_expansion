...
class ImageExpansion:
    def __init__(self):
        ...
        self.BGI_PATH = '/backgroundImage/' #背景画像のパス
    ...
    def extractObjectArea(self, img):
        ...
        while input_key != ord('s'):
            wb_img = cv2.imread(self.PKG_DIR + '/wb/white_back.jpg', 1)
            ...
            input_key = self.viewImage(obj_wb_img)
            if input_key == ord('r'):
                self.OBJ_GAMMA += 0.05
                ...
                bb_img = cv2.imread(self.PKG_DIR + '/bb/blue.jpg', 1)
